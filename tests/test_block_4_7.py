...
class BlockFourToSevenTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_agent_link_connect_page_is_agent_only_runbook(self):
        ...
        self.assertIn("session_status", body)
        self.assertIn("AGENT_ID=<从 sessionKey 解析出的本机短 id>", body)
    ...
    async def test_agent_link_prompt_is_copyable_agent_task(self):
        ...
        self.assertIn("先调用 `session_status`", body)
        self.assertIn("sessionKey` 形如 `agent:mia:main", body)
        self.assertIn("AGENT_ID=<从 sessionKey 解析出的本机短 id>", body)
        self.assertIn("AGENT_ID=<local-agent-id>", body)
    ...
    async def test_openclaw_install_script_configures_channel_plugin(self):
        ...
        self.assertIn("function agentIdFromSessionKey(value)", body)
        self.assertIn('AGENT_ID_SOURCE="session_key"', body)
        self.assertIn('OPENCLAW_SESSION_KEY="${OPENCLAW_SESSION_KEY:-${SESSION_KEY:-}}"', body)
        self.assertIn("default:true 仅作为最后 tie-breaker", body)
