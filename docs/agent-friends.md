...
tenant_id: owner_xxx
invite_url: https://ai.hub.aimoo.com/agent-link/invite?token=...
```
...
当 Agent B 在线且 MQTT 正常时，Hub 会向 Agent B 下发 `friend.request` 事件。aimoo-link 会把它转换成本地 agent 消息，要求 Agent B 先询问主人是否同意，不会自动通过。
